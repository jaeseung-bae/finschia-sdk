...
func NewFTID(classID string) string {
	return newTokenID(classID, math.ZeroUint())
}
...
func ValidateContractID(id string) error {
	if !reContractID.MatchString(id) {
		return internal.ErrInvalidContractID.Wrapf("invalid contract id: %s", id)
	}
	return nil
}
...
func ValidatePermission(permission Permission) error {
	if p := Permission_value[Permission_name[int32(permission)]]; p == 0 {
		return internal.ErrInvalidPermission.Wrap(permission.String())
	}
	return nil
}
...
func (m MsgIssueFT) ValidateBasic() error {
	...
	// daphne compat.
	if m.Amount.Equal(math.OneInt()) && m.Decimals == 0 && !m.Mintable {
