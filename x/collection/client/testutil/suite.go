...
var commonArgs = []string{
	...
	fmt.Sprintf("--%s=%s", flags.FlagFees, sdk.NewCoins(sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(100))).String()),
}
...
func (s *IntegrationTestSuite) mintFT(contractID string, operator, to sdk.AccAddress, classID string, amount math.Int) {
	val := s.network.Validators[0]
	args := append([]string{
		contractID,
		operator.String(),
...
func (s *IntegrationTestSuite) createAccount(uid string) sdk.AccAddress {
	...
	fee := sdk.NewCoins(sdk.NewCoin(s.cfg.BondDenom, math.NewInt(1000000)))
	args := append([]string{
		val.Address.String(),
		addr.String(),
		fee.String(),
